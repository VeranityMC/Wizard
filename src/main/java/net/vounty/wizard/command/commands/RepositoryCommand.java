    ...
    switch (arguments.size()) {
        ...
        switch (arguments.get(0).toLowerCase()) {
          case "list", "l" -> {
            ...
            repositories.forEach(repository ->
              this.getLog()
                .info(" §b{0}§r (ID: §1{1}§r, Visible: §1{2}§r, MultipleDeployments: §1{3}§r)",
                  repository.getName(),
                  repository.getUniqueId(),
                  repository.getVisible(),
                  repository.getMultipleDeployments()));
          }
          default -> {
            return true;
          }
      ...
      case 2 -> {
        ...
        switch (arguments.get(0).toLowerCase()) {
          ...
          case "create", "c" -> {
            optionalRepository.ifPresentOrElse(repository -> this.getLog()
              .warn("Repository with name '{0}' already exist.", name), () -> {
              final var newRepository = new WizardRepository(name);
              final var result = this.getWizard()
          ...
          case "drop", "d" -> {
            optionalRepository.ifPresentOrElse(repository -> {
              ...
              this.getLog().log(result ? LogState.INFO : LogState.ERROR,
                result ? "Repository §b{0}§r was successfully removed." : "Unable to remove repository §b{0}§r. Try again later.",
                name);
            }, () -> this.getLog().error("Cannot find repository with name or id '§b{0}§r'", name));
          }
          default -> {
            ...
            optionalRepository.ifPresentOrElse(repository -> {
              switch (arguments.get(2).toLowerCase()) {
                case "visible", "v" -> {
                  ...
                  this.getLog()
                    .info("The repository §b{0}§r is now §b{1}§r.", name, repository.getVisible());
                }
                case "multipledeployments", "multideploy", "multipledeploy", "multideployments",
                     "md" -> {
                  repository.toggleMultipleDeployments();
                  this.getLog()
                    .info("Multiple deployments of repository §b{0}§r are now §b{1}§r.",
                      name,
                      repository.getMultipleDeployments() ? "enabled" : "disabled");
                }
              ...
              switch (action.toLowerCase()) {
                case "name", "n" -> {
                  ...
                  switch (nameResult) {
                    ...
                    case ALREADY_EXIST -> this.getLog()
                      .warn("Repository with name '{0}' already exist.", repository.getName());
                  }
                }
                default -> this.getLog().warn("Use '§brepository§r' for more help.");
              }
              ...
              optionalToken.ifPresentOrElse(token -> {
                ...
                switch (action.toLowerCase()) {
                  case "add", "a" -> {
                    ...
                    switch (tokenResult) {
                      case SUCCESS -> this.getLog()
                        .info("Token §b{0}§r was added to repository §b{1}§r.",
                          value,
                          repository.getName());
                      case ALREADY_EXIST -> this.getLog()
                        .warn("Token §b{0}§r already exist in repository §b{1}§r.",
                          value,
                          repository.getName());
                    }
                  }
                  case "drop", "d" -> {
                    ...
                    switch (tokenResult) {
                      case SUCCESS -> this.getLog()
                        .info("Token §b{0}§r was removed from repository §b{1}§r.",
                          value,
                          repository.getName());
                      case NOT_EXIST -> this.getLog()
                        .warn("Token §b{0}§r does not exist in repository §b{1}§r.",
                          value,
                          repository.getName());
                    }
                  }
