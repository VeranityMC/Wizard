...
public class TokenCommand extends WizardCommand {
        ...
        switch (arguments.get(0).toLowerCase()) {
          case "about", "a" -> {
            optionalToken.ifPresentOrElse(token -> {
              ...
              final var repositories = this.getWizard()
                .getTokenAdapter()
                .getRepositoriesFromToken(token);
              if (repositories.size() == 0) {
                this.getLog().info("Repositories: §cEmpty§r");
                return;
              }
              this.getLog().info("Repositories (§1{0}§r)", repositories.size());
              repositories.forEach(repository -> this.getLog()
                .info(" §b{0}§r", repository.getName()));
            }, () -> this.getLog().error("Cannot find token with name or id '§b{0}§r'", name));
          }
          case "create", "c" -> {
            optionalToken.ifPresentOrElse(token -> this.getLog()
              .warn("Token with name/id '{0}' already exist.", name), () -> {
              ...
              if (result)
                this.getLog()
                  .info("Token §b{0}§r was successfully created. (Password: §c{1}§r)",
                    name,
                    password);
          ...
          case "toggle", "t" -> {
            optionalToken.ifPresentOrElse(token -> {
              token.toggleStatus();
              this.getLog()
                .info("The token §b{0}§r is now §b{1}§r.",
                  name,
                  token.getActive() ? "active" : "inactive");
            }, () -> this.getLog().error("Cannot find token with name or id '§b{0}§r'", name));
          }
          default -> {
            return true;
                ...
                case "name", "n" -> {
                  ...
                  switch (nameResult) {
                    ...
                    case ALREADY_EXIST -> this.getLog()
                      .warn("Token with name '{0}' already exist.", token.getUserName());
                  }
                }
                case "password", "p" -> {
                  ...
                  final var isGenerate = value[0].equalsIgnoreCase("--generate") || value[0].equalsIgnoreCase(
                    "--gen");
  ...
  public void displayUsage() {
    ...
    this.getLog()
      .usage(
        "Use '§btoken modify (§1Name§b) name, password (§1Value §8{§b--generate§r, to generate password§8} §b)§r'");
  }
