...
public class WizardRepository implements Repository {
  ...
  @Override
  public void download(HttpServletRequest request, Token token, Framework framework) throws
    IOException {
    ...
    if (folder.exists() && !this.getMultipleDeployments()) {
      Wizard.getService()
        .getLog()
        .warn(
          "User §b{0}§r (§1{1}§r) tried to deploy on §b{2}§r but multi deployments are disabled.",
          token.getUserName(),
          address,
          this.getName());
      Spark.halt(403);
    ...
    final var deploy = this.deployments.getOrDefault(address,
      new RepositoryDeploy(folder, new LinkedList<>()));
    ...
    Executors.newSingleThreadScheduledExecutor().schedule(() -> {
      ...
      final var lastDeployed = this.lastDeployment.getOrDefault(address,
