...
public class WebServer implements Server {
  ...
  public void start(Consumer<String> host) {
    ...
    Spark.port(configuration.getSsl().getEnabled() ? configuration.getSsl()
      .getPort() : configuration.getProtocol().getPort());
    if (configuration.getSsl().getEnabled())
      Spark.secure(configuration.getSsl().getKeystoreFilePath(),
        configuration.getSsl().getKeystorePassword(),
        configuration.getSsl().getTruststoreFilePath(),
        configuration.getSsl().getTruststorePassword());

    Spark.threadPool(configuration.getThreads().getMax(),
      configuration.getThreads().getMin(),
      configuration.getThreads().getTimeoutMillis());
