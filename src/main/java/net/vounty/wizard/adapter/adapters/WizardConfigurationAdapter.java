...
public class WizardConfigurationAdapter extends WizardAdapter implements ConfigurationAdapter {
  ...
  public void generateConfigurations() {
    ...
    this.saveConfiguration(PathState.REPOSITORIES_CONFIG,
      RepositoriesConfiguration.getDefault(),
      false);
  ...
  public void loadConfigurations() {
    this.setWizardConfiguration(this.readConfiguration(PathState.WIZARD_CONFIG,
      WizardConfiguration.class));
    this.setTokensConfiguration(this.readConfiguration(PathState.TOKENS_CONFIG,
      TokensConfiguration.class));
    this.setRepositoriesConfiguration(this.readConfiguration(PathState.REPOSITORIES_CONFIG,
      RepositoriesConfiguration.class));
    this.setWebsiteConfiguration(this.readConfiguration(PathState.WEBSITE_CONFIG,
      WebsiteConfiguration.class));
    this.setSecureConfiguration(this.readConfiguration(PathState.SECURE_CONFIG,
      SecureConfiguration.class));
  }

  @Override
  public void saveConfigurations() {
    this.saveConfiguration(PathState.TOKENS_CONFIG,
      new TokensConfiguration(this.getWizard()
        .getTokenAdapter()
        .getTokens()
        .stream()
        .map(token -> (WizardToken) token)
        .toList()),
      true);
    this.saveConfiguration(PathState.REPOSITORIES_CONFIG,
      new RepositoriesConfiguration(this.getWizard()
        .getRepositoryAdapter()
        .getRepositories()
        .stream()
        .map(repository -> (WizardRepository) repository)
        .toList()),
      true);
    this.saveConfiguration(PathState.SECURE_CONFIG,
      new SecureConfiguration(this.getWizard().getSecureAdapter().getAddresses()),
      true);
  }
  ...
  public <Value> Value readConfiguration(File file, Class<Value> type) {
    try {
      final var inputStreamReader = new InputStreamReader(new FileInputStream(file),
